...
struct GeneRaxInstance {
  ...
  GeneRaxInstance(int argc, char **argv)
      : args(argc, argv), totalLibpllLL(0), totalRecLL(0), elapsedRates(0),
        elapsedSPR(0), elapsedRaxml(0), currentIteration(0) {
    auto recModel = ArgumentsHelper::strToRecModel(args.reconciliationModelStr);
    std::string fractionMissingFile;
    recModelInfo = RecModelInfo(
        recModel, RecOpt::Gradient, args.perFamilyDTLRates,
        args.gammaCategories, args.originationStrategy, args.pruneSpeciesTree,
        args.rootedGeneTree, args.forceGeneTreeRoot, args.madRooting,
        args.minGeneBranchLength, args.transferConstraint, args.noDup,
        false, // no TL
